...
export function Dashboard({ data, source, onRefresh, onResetDemo }: Props) {
  ...
  return (
    <section className="dashboard-screen screen-wrap">
      <div className="dashboard-title"><div><p className="pixel-kicker">OPERATION ANALYTICS</p><h1>トラブル集計</h1><p>件数だけでなく、対応レベル・時間・再発性から改善対象を探します。</p></div><div className="dashboard-actions"><button className="refresh-button" onClick={onRefresh}>↻ 更新</button>{source === "demo" && <button className="demo-reset-button" onClick={onResetDemo}>初期状態へ戻す</button>}</div></div>
      ...
      <div className="dashboard-columns">
        <article className="pixel-window priority-window">
          <header><div><span>REPAIR HINTS</span><h2>重点改善候補</h2></div><small>対応レベル × 時間 × 再発係数</small></header>
          <div className="priority-list">{data.priorities.map((item, index) => <div className="priority-item" key={`${item.scenarioTitle}-${index}`}>
            <span className="rank">{index + 1}</span><div className="priority-copy"><p><strong>{item.scenarioTitle}</strong><span>{item.areaName}・{item.count}件</span></p><div className="pixel-bar"><i style={{ width: `${Math.max(7, (item.score / maxScore) * 100)}%` }} /></div></div><b>{item.score}<small>SCORE</small></b>
          </div>)}</div>
          <p className="priority-note">改善優先スコアは候補同士を比較する目安です。数値だけで改修を決めず、記録内容と安全性を合わせて確認します。</p>
        </article>
        ...
        <article className="pixel-window unknown-window">
          ...
          <p>同じ「想定外」が重なったら、新しい対応フローとして追加する候補です。</p>
        </article>
      </div>

      <article className="pixel-window history-window">
        <header><div><span>RECENT LOG</span><h2>最近の対応記録</h2></div></header>
