...
export function Dashboard({ data, source, onRefresh, onResetDemo }: Props) {
  ...
  const completeRecovery = async () => {
    ...
    try {
      ...
      window.alert("対応完了の記録を保存できませんでした。接続状態を確認してください。");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <section className="dashboard-screen screen-wrap">
      <div className="dashboard-title"><div><p className="pixel-kicker">OPERATION STATUS</p><h1>トラブル集計</h1><p>最初に対応中と安全懸念を確認し、その後で改善候補を見ます。</p></div><div className="dashboard-actions"><button className="refresh-button" onClick={onRefresh}>↻ 更新</button>{source === "demo" && <button className="demo-reset-button" onClick={onResetDemo}>初期状態へ戻す</button>}</div></div>

      <article className={`urgent-board ${data.activeSummary.total > 0 ? "has-alerts" : "all-clear"}`}>
        <header><div><span>NOW</span><h2>今すぐ見るもの</h2></div><p>{data.activeSummary.total > 0 ? "対応中・安全懸念が残っています" : "対応中の記録はありません"}</p></header>
        ...
        </div> : <div className="no-active">✓ 現在、対応中として残っている記録はありません。</div>}
      </article>
      ...
      <div className="metric-grid">
        ...
        <article className="metric-card blue"><span>TO COMPLETE</span><strong>{data.averageRecoveryMinutes}<em>分</em></strong><small>完了まで平均</small></article>
      </div>

      <div className="dashboard-columns">
        <article className="pixel-window priority-window">
          <header><div><span>REPAIR HINTS</span><h2>重点改善候補</h2></div><small>対応レベル × 完了時間 × 再発性</small></header>
          <div className="priority-list">{data.priorities.map((item, index) => <div className="priority-item" key={`${item.scenarioTitle}-${index}`}>
            <span className="rank">{index + 1}</span><div className="priority-copy"><p><strong>{item.scenarioTitle}</strong><span>{item.areaName}・{item.count}件</span></p><div className="reason-chips"><span>再発 {item.repeatCount}</span><span>停止 {item.stoppedCount}</span><span>完了平均 {item.averageRecoveryMinutes}分</span></div><div className="pixel-bar"><i style={{ width: `${Math.max(7, (item.score / maxScore) * 100)}%` }} /></div></div><b>{item.score}<small>SCORE</small></b>
          </div>)}</div>
      ...
      <article className="pixel-window history-window">
        ...
        <div className="log-table-wrap">
          <table>
            <thead><tr><th>日時</th><th>エリア</th><th>トラブル</th><th>状態</th><th>完了</th><th>再発</th></tr></thead>
            <tbody>{data.recent.map((item) => (
              <tr key={item.id} className="clickable-row" tabIndex={0} role="button" onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { setSelectedIncident(item); setResolutionNote(""); } }} onClick={() => { setSelectedIncident(item); setResolutionNote(""); }}>
                ...
                <td data-label="完了">{formatMinutes(item.recoverySeconds)}</td>
      ...
      {selectedIncident && <div className="detail-overlay" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) setSelectedIncident(null); }}>
        <article className="detail-dialog" role="dialog" aria-modal="true" aria-label="対応記録の詳細">
          ...
          <div className="detail-body">
            <div className="detail-facts"><p><span>状態</span><strong className={`result-chip ${selectedIncident.status === "open" ? "open" : selectedIncident.result}`}>{selectedIncident.status === "open" ? "対応中" : "対応完了"}</strong></p><p><span>発生</span><strong>{formatDate(selectedIncident.occurredAt)}</strong></p><p><span>切り分け</span><strong>{formatMinutes(selectedIncident.triageSeconds)}</strong></p><p><span>完了まで</span><strong>{formatMinutes(selectedIncident.recoverySeconds)}</strong></p></div>
            ...
            {selectedIncident.resolutionNote && <section><h3>完了内容</h3><p>{selectedIncident.resolutionNote}</p></section>}
            {selectedIncident.status === "open" && <div className="recovery-form"><label><span>完了内容 *</span><textarea rows={3} value={resolutionNote} onChange={(event) => setResolutionNote(event.target.value)} placeholder="対応完了を確認した事実と実施内容" /></label><button className="pixel-primary" disabled={!resolutionNote.trim() || saving} onClick={completeRecovery}>{saving ? "保存中..." : "対応完了として記録"}</button></div>}
          </div>
        </article>
      </div>}
