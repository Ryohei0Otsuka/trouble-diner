...
公開デモはPHP・MySQL不要で、そのまま操作できます。対応記録・未分類トラブル・追加フローは閲覧者自身のブラウザにだけ保存され、集計画面から初期状態へ戻せます。

[制作メモをnoteで読む](https://note.com/happy_knot260/n/na5ac547e70de)
...
- フローの安全区分と結果による対応レベルの自動判定
- 確認経路を含む引き継ぎ票の生成
- 対応レベル・時間・再発性による改善優先スコア
- 想定外トラブルを蓄積する未分類ボックス
...
- 実行時の対応レベルを作業者に変更させず、フロー結果から自動判定する
